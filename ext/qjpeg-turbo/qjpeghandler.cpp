                            ...
                            QRect clipRect, int inQuality, j_decompress_ptr info, struct my_error_mgr* err  )
{
    if (!setjmp(err->setjmp_buffer)) {
        ...
        volatile int quality = inQuality;
        if (quality < 0)
            quality = 75;
        ...
        if (clip == imageRect && (info->output_components == 1
            ...
            while (info->output_scanline < info->output_height) {
                ...
                if (info->output_components == 3 && info->out_color_space == JCS_RGB) {
                    ...
                    QRgb *out = reinterpret_cast<QRgb*>(outImage->scanLine(y));
...
static bool write_jpeg_image(const QImage &image, QIODevice *device, int sourceQuality)
{
    ...
    if (!setjmp(jerr.setjmp_buffer)) {
        ...
        volatile int quality = sourceQuality >= 0 ? qMin(sourceQuality,100) : 75;
#if defined(Q_OS_UNIXWARE)
        ...
        while (cinfo.next_scanline < cinfo.image_height) {
            ...
            case QImage::Format_ARGB32_Premultiplied:
                {
                    const QRgb* rgb =
                            reinterpret_cast<const QRgb*>(image.constScanLine(cinfo.next_scanline));
                    for (int i=0; i<w; i++) {
                        *row++ = qRed(*rgb);
                        *row++ = qGreen(*rgb);
                        *row++ = qBlue(*rgb);
            ...
            default:
                {
                    ...
                    const QRgb* rgb =
                            reinterpret_cast<const QRgb*>(rowImg.constScanLine(0));
                    for (int i=0; i<w; i++) {
                        *row++ = qRed(*rgb);
                        *row++ = qGreen(*rgb);
                        *row++ = qBlue(*rgb);
