...
public:
    ...
    // Static function returning the texture coords for the full texture: [0] = topLeft (0,0),
    // [1] = topRight (1,0), [2] = bottomRight (1,1) and [3] = bottomLeft (0,1).
    static RectUVCoords defaultTextureCoords();

    bool operator==(const STTexturedQuads& rhs) const;
    bool operator!=(const STTexturedQuads& rhs) const;
