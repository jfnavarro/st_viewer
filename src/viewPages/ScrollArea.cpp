...
void ScrollArea::initializeView(QPointer<CellGLView> view)
{
    ...
    connect(m_view.data(),
            SIGNAL(signalViewPortUpdated(const QRectF)),
            this,
            SLOT(setCellGLViewViewPort(const QRectF)));
    connect(m_view.data(),
            SIGNAL(signalSceneUpdated(const QRectF)),
            this,
            SLOT(setCellGLViewScene(const QRectF)));
    connect(m_view.data(),
            SIGNAL(signalSceneTransformationsUpdated(const QTransform)),
            this,
            SLOT(setCellGLViewSceneTransformations(const QTransform)));

    connect(verticalScrollBar(),
            SIGNAL(sliderMoved(int)),
            this,
            SLOT(someScrollBarChangedValue(int)));
...
void ScrollArea::adjustScrollBars()
{
    ...
    if (m_cellglview_scene.isValid() && m_cellglview_viewPort.isValid()
        ...
        adjustScrollBar(m_scrollBarSteps,
                        focusPointF.x(),
                        allowedRectF.x(),
                        allowedRectF.width(),
                        viewPortInSceneCoordinates.width(),
                        horizontalScrollBar());
        ...
        adjustScrollBar(m_scrollBarSteps,
                        ...
                        allowedRectF.y(),
                        allowedRectF.height(),
                        viewPortInSceneCoordinates.height(),
                        verticalScrollBar());
    }
}
